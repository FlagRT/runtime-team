...
（含 **910C 第一实例**的能力撒谎与 `info()` 键名漂移；涉事后端属路线 B，已随路线 B 整体删除，判据保留以守护现役三家）
⇒ 该阻塞**已消解**：910C 网络已恢复，涉及的后端随后随**路线 B 整体删除**（见下方 09-22 收口追加行）
...
　　　**同日追加（09-22 傍晚）：三芯片职责验收完成，原型可发布** —— 按《接口约定》的五域 +
　　《多流基线》16 项，在**厂商 torch 插件统一原型**下逐芯片重跑 10 项判定：
　　**910C 10/10 通过 · P800 10/10 通过**（conformance 13/13+6/6、多流 8/8+4/4+3/3、
　　两条腿 6/6 与 14/14 / 13/13、错误闭环 5/0/0、服务化 `SERVE_STANDARD_PASS`）；
　　**MLU590 两台主机当日 SSH 无响应** ⇒ 推理腿与服务化 2 项未做，**本轮不参与发布判定**
　　（代码层与判据层已就绪）。验收过程另修 **3 处缺陷**（探针缺"触碰设备"、两条腿 P800 用法示例两处错、
　　`serve_standard.sh` 不能起同形态服务 ⇒ 加 `SERVE_FORM` 覆盖）。详见
　　`prototype/docs/PROTOTYPE_ACCEPTANCE_3CHIP_20260922.md`。
　　⚠️ 另实测确认：**910C 训练容器与推理容器不能同时持卡**（二者都挂全部 16 设备 ⇒ 后起方引擎起不来；
　　**与"并发上限 3"是两件事**，本轮仅 2 个带卡容器仍未超名额）⇒ **两条腿串行**。
　　**同日追加（09-22 收口）：路线 B（torch_fl）整体退出** —— 原型里的该后端已**删除**，
注册表 / 自检工具 / 文档 / 复跑命令一并清理；910C 训练腿此前已统一 `npu`（torch_npu）。
⇒ **三家芯片实例（910C / P800 / MLU590）当前一律走厂商官方 torch 插件路线**
（`torch_npu` / `torch.cuda` 兼容层 XPytorch / `torch_mlu`），原型内**不再有该路线的活跃路径**。
