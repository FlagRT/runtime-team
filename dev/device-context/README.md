...
> 分支：`kistich/device-context` ｜ PR 目标：`dev-1.0` ｜ 更新：**2026-09-22**
...
>
> 当前状态：**两实例（昇腾 910C / 昆仑芯 P800）阶段 0–5 全部完成，并已在当前代码上完成全量对称复核**
> （smoke 51/0 · conformance 13+6 双侧全绿 · 语义基线 8/8 · 两条腿 14/14 与 6/6 · 错误闭环 5/0/0）。
...
| 目录 | 定位 | 内容 |
|---|---|---|
| **`prototype/`** | **芯片无关的统一标准**（我们制定的规范、实现与判据） | 统一运行时 API、Backend 注册表与后端实现（`ascend` / **`kunlun`** / `cambricon`）、conformance 用例与 runner、两条腿自验证脚本、**接口约定与事件语义契约**、原型设计、月度计划、**三芯片职责验收报告**（`docs/PROTOTYPE_ACCEPTANCE_3CHIP_20260922.md`） |
...
| **`MLU590/`** | **第三个接入实例（寒武纪）** —— ✅ **接入完成**（环境打通 → 镜像定档 → 后端落地 → 真机验证全绿 → 16 项基线 → 训练腿 → 错误闭环）；⏳ 判据口径调整后的**新口径新鲜证据待补**（寒武纪网络中断） | 环境报告、镜像渠道调研与更正、**接入方案 + 真机执行手册**、**16 项基线比对报告**、探针脚本与原始证据（`docs/`、`probes/`） |
...
| **《组内服务启动标准》** | ✅ | `prototype/scripts/serve_standard.sh`（**下游起服务唯一入口**，跨芯片只改 `DC_BACKEND`）+ `prototype/docs/SERVICE_STARTUP_STANDARD_20260920.md`；**910C / P800 真机均 `SERVE_STANDARD_PASS`**（910C 就绪 30 s + 生成冒烟 8 tokens；P800 就绪 25 s + 冒烟维度 1024）；**09-22 三实例同形态验收**（新增 `SERVE_FORM=embed`）—— 910C 35 s、P800 25 s 就绪，均 PASS |
...
| ⭐ **三芯片职责验收（09-22 傍晚）** | ✅ **10 项全绿** | 离线 35/0 · 对称性 5/0 · 冒烟 52/0 · conformance 13/13+6/6 · 多流 8/8+4/4+3/3 · 训练腿 6/6 · 推理腿 14/14 · 服务化 PASS · 错误闭环 5/0/0 ⇒ 见 `prototype/docs/PROTOTYPE_ACCEPTANCE_3CHIP_20260922.md` |
...
| 项 | 状态 | 证据 |
|---|---|---|
| ⭐ **三芯片职责验收（09-22 傍晚）** | ✅ **10 项全绿** | 离线 39/0 · 对称性 5/0 · 冒烟 **46/0** · conformance **13/13+6/6** · 多流 **8/8**（S12 如实不支持）+ 图捕获 4/4 + 配额 3/3 · 训练腿 **6/6**（**3533.5 tok/s**）· 推理腿前向 **13/13+1 跳过**（53.28 句/s、p50 56.12 ms）· 服务化 **`SERVE_STANDARD_PASS`**（25 s 就绪）· 错误闭环 **5/0/0** ⇒ 见 `prototype/docs/PROTOTYPE_ACCEPTANCE_3CHIP_20260922.md` |
...
### 2.5 MLU590 实例（第三个接入实例，✅ **接入完成** · 2026-09-22）：环境/镜像 → 后端落地 → conformance 13+6 → **多流 16 项（15 通过 / 1 不适用）** → **训练腿 6/6（2957.8 tok/s）** → **错误闭环 5/0/0**
...
| 多流 16 项 / 训练腿 / 错误闭环 | ✅ **09-22 完成** | 多流 **15 通过 / 1 不适用 / 0 不支持**；训练腿 **6/6**（2957.8 tok/s，`dist=cncl`）；错误闭环 **5/0/0** |
| ⚠️ **三芯片职责验收（09-22 傍晚）未连通** | ⏳ **本轮不参与发布判定** | 两台主机（10.1.1.21 / 10.1.1.22）当日 **SSH 均超时**（同刻 910C / P800 正常）⇒ **推理腿前向与服务化 2 项未做**；cambricon 后端**代码层与判据层已就绪**，主机可达后按同一套命令补齐即可并入。如实登记，未以历史数据顶替 ⇒ 见 `prototype/docs/PROTOTYPE_ACCEPTANCE_3CHIP_20260922.md` |
...
| 20 | **多流 Stream 16 项基线**（第三实例）：**15 通过 / 1 不适用 / 0 不支持**；探针 **8/8**（双卡含 S-13）、图捕获 **5/5**、S-16 配额 **2000 流 3/3**；**S-12 流优先级 MLU590 支持（`(0,-3)`）而 P800 不支持** ⇒ 同一 API 跨芯片相反 | ✅ **本轮** |
| 21 | **第 3 家 · 训练腿 2 卡**：**`TRAIN_LEG_PASS 6/6`**、loss **15.4498 → 11.1479**（50 步无 NaN）、**2957.8 tok/s**、集合通信后端名 **`cncl`**（三类对照全对）。⚠️ 走 **MLU_LINK 片间互联**、**非 RDMA**（CNCL 未加载 `libibverbs`/`libmlx5`） | ✅ **本轮** |
| 22 | **第 3 家 · 错误闭环**：四类注入 **`ERROR_RECOVERY_LOOP_PASS`（闭环 5 / 跳过 0 / 失败 0）** | ✅ **本轮** |
| 23 | **跨后端对称性审计**（第 3 家接入的收口产出）：挖出 **3 个跨后端缺陷 + 2 处证据污染** —— ① 无码表后端"诚实降级"只降 `graded_by`、`mapped`/`error_code` 未降 ⇒ **假"确定分级"**（`kunlun`/`cambricon` 同款）；② **某后端声明 `device_state` 却无实现**（910C 训练腿当时所用后端，属路线 B，已删除）；③ 该后端的 `info()["supports"]` 手写第二份键名清单 ⇒ 已声明能力恒显 False。**离线自检工具扩到三家可用 + 显式 SKIP 机制 + 崩溃改判 FAIL**，并加 4 条防回归判据（均做过**非空转验证**） | ✅ |
| 24 | **910C 复核（网络恢复后）暴露 2 个新缺陷**：④ **`ascend` 的 `acl.init()` 返回值未检查** ⇒ 把"ACL 初始化/参数错误"**冒充成"设备同步超时"**（真机链：`acl.init`=500000 → `set_device`=107002 → `sync`=107000 → 却抛 `TimeoutError`；下游会按 L3 去 `replay`，**动作反了**）。修复=按 rc 分级，**只有已知超时码**才映射 `TimeoutError`；错误码表增补 `500000`（注明来源 `acl_base_rt.h`）；为 `ascend` 登记首条 `known_issues`。⑤ **统一错误对象 `FlagosError` 跨层类型不一致**（`api` 层是纯 dataclass **不能 `raise`**、`conformance` 层是 `Exception`）⇒ 修为继承 `Exception`。**两处均由复核挖出，且 ⑤ 是修 ④ 时才暴露的** | ✅ **本轮** |
| 26 | ⭐ **910C 训推口径统一（本轮决定项）**：全组统一基座 = Qwen3-0.6B **训推** + 原型接入的**厂商 torch 插件路线** ⇒ 训练腿由路线 B（torch_fl）切到 **`npu`（torch_npu）**，**v1 的「权宜例外」正式取消**。做法：用容器内带 torch_npu 的解释器（`venv-infer-a`，**无 torch_fl ⇒ 物理隔离**，不触发镜像"禁止共存"校验）；**镜像本身未切换**。实测（2 卡/50 步/Qwen3-Embedding-0.6B）：`TRAIN_LEG_PASS 6/6`、loss **15.4498→11.1479**（torch_fl 线 15.4497→11.1515）、**3954–4402 tok/s**（torch_fl 线 2212.9 ⇒ **+79~99%**）；统一口径全量复核：smoke **52/0** · conformance **13/13+6/6** · 训练腿 **6/6** · 错误闭环 **5/0/0**。**过程中又挖出第 15 条缺陷**：`hccl` 后端名要厂商扩展 import 后才注册，`init_process_group("hccl")` 在 `use()` 之前会报 `Unknown backend type hccl` | ✅ **本轮** |
...
| 25 | **自检工具自身两处问题**（都会伪装成"后端没问题"）：⑥ **"离线"自检并不离线**（stub 只换 `torch`，真机 `PYTHONPATH` 上的真实 `acl` 被 import 并访问硬件 ⇒ 910C 上直接 traceback）⇒ 新增**真实厂商运行时阻断器** + 为 `ascend` 注入**可控 rc 的假 pyACL**，并把"未以真实文件形式加载任何厂商运行时"设为**判据**；⑦ **未预期异常让整轮无汇总地崩掉**（踩到 2 次）⇒ 入口统一兜底。另新增 **`--all` 跨后端对称性自检**（硬判据 5 条 + 差异清单，当前 **5/0**），并补齐 `ascend` 的 `_CAPABILITY_KEYS` + `info()`（它此前是四家中唯一没有 `supports` 映射的） | ✅ **本轮** |
| 26 | **验证资产口径更正**：P800 报告 §3.1 的"S-7 按 910C 同口径 5/5"**不成立**（那 5 项与 910C 的 G1–G5 不是同一组判据，且**脚本与 JSON 均未归档**、不可复现）。按归档口径复跑：P800 **契约内 4/4 通过** + 1 项**上游契约外用法**不容忍；把「捕获区内切流」从判据**降为宽容度观察项**（MLU590 容忍 / P800 不容忍 ⇒ **非能力差异**），并加**条目隔离**（失败捕获会污染后续条目） | ✅ **本轮** |
| 27 | **P800 全量真机回归**（改动后）：离线 **39/0** · 对称性 **5/0** · smoke **46/0** · conformance **13/13 + 6/6** · 错误闭环 **5/0/0** · 图捕获契约内 **4/4** · 配额 **3/3** | ✅ **本轮** |
| 28 | **待补（外部阻塞）**：910C 真机复验被**带卡容器并发名额占满**阻塞（他人 4 个容器；`acl.init`=500000、芯片却 `Health: OK`）；寒武纪两台主机**网络中断** ⇒ MLU590 的新口径新鲜证据待补 | ⛔ **待协调** |
...
| `prototype/docs/event_semantics_contract.md`（54 行） | **事件语义契约 E1–E4**（昇腾实测驱动的 v2 修订）：事件/流的同步与依赖语义边界 | 同上 | 同上 |
| `prototype/docs/SERVICE_STARTUP_STANDARD_20260920.md`（236 行） | **你们必须怎么起服务**：唯一入口 `serve_standard.sh`、参数表、**六条硬纪律**、三个已知行为、与自建脚本的关系 | 所有需要在国产芯片上起服务的方向与验收方 | 改脚本 + 改文档 → **两实例各跑一次** → 知会下游 |
| `prototype/docs/INTERFACE_CONTRACT_REVISION_PROPOSAL_20260920.md`（337 行） | **接口约定修订建议 6 条**（P800 是现行约定的首次非昇腾检验）：`device_type`/`vendor` 分离 · `device_state` 入契约 · `.native` 逃生舱约束 + `record_stream` 能力位 · **错误对象跨模块类归一** · 有界同步降级语义 · `known_issues()` 入契约。每条含 ①现状 ②实测依据（带源码行号）③建议条文 ④兼容性 | 接口约定评审人、全部下游 | **尚未生效**——待裁定后并入接口约定，届时按 6.1 第 1 行走流程 |
...
| `DC_STAGE_SUMMARY_20260909.md`（162 行） | 设备上下文阶段性总结：两条腿证据并入 |
| `ERROR_RECOVERY_LOOP_20260909.md`（138 行） | 错误注入 → 恢复闭环验证记录（含一处归因核查被推翻的记录） |
| `DIAG_TRAIN_IMAGE_NPU_20260908.md`（64 行） | 训练腿锁定镜像 NPU 初始化失败排查记录 |
| `DEVICE_CONTEXT_PLAN_20260827.md`（117 行） | 设备执行上下文方案定稿（8 月版） |
| `PROGRESS_20260822.md`（75 行） | 8-22 阶段进度快照 |
...
| `KUNLUN_P800_ADAPT_PLAN_20260914.md`（444 行） | **接入工作方案**：定位与交付边界、接入路线、验收 6 条、阶段计划、风控；**§7.5 = 接入过程暴露的 3 个「只有非昇腾实例才暴露」的框架缺陷** |
| `KUNLUN_P800_ROOT_CAUSE_VERIFY_20260914.md`（202 行） | 集合通信挂死结论核对与**责任层判定**（两要素条件、函数级定位到厂商 `.so`） |
