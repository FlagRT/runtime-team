...
| **`MLU590/`** | **第三个接入实例（寒武纪）** —— ✅ **接入完成**（环境打通 → 镜像定档 → 后端落地 → 真机验证全绿 → 16 项基线 → 训练腿 → 错误闭环） | 环境报告、镜像渠道调研与更正、**接入方案 + 真机执行手册**、**16 项基线比对报告**、探针脚本与原始证据（`docs/`、`probes/`） |
...
| 23 | **跨后端对称性审计**（第 3 家接入的收口产出）：挖出 **3 个跨后端缺陷 + 2 处证据污染** —— ① 无码表后端"诚实降级"只降 `graded_by`、`mapped`/`error_code` 未降 ⇒ **假"确定分级"**（`kunlun`/`cambricon` 同款）；② **`flagos` 声明 `device_state` 却无实现**（910C 训练腿后端）；③ `flagos` 的 `info()["supports"]` 手写第二份键名清单 ⇒ 已声明能力恒显 False。**离线自检工具扩到四家可用 + 显式 SKIP 机制 + 崩溃改判 FAIL**，并加 4 条防回归判据（均做过**非空转验证**） | ✅ **本轮**（910C 侧真机复验**待网络恢复**） |
