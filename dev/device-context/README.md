...
| 目录 | 内容 |
|---|---|
| `prototype/docs/` | **芯片无关的统一标准**：接口约定（**新芯片接入规范**）、事件语义契约、原型设计、职责框架、月度计划、通信路线；**《新芯片接入手册》**（接芯片先读这份）；**《组内服务启动标准》**（起服务先读这份）；**《两实例验证复核清单》**（复核先读这份，含缺口 G1–G8）；**接口约定修订建议 6 条**；另有三份**跨实例参考**——**两实例验证配置与依据**（镜像/模型/训推框架/参数的逐项依据链 + 可复现命令）、**镜像选择与确定指南**、**镜像需求说明书**（提交总组，含硬性/期望/可协商三级需求） |
