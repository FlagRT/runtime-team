...
| **错误注入 → 恢复闭环**（验收标准 3） | 🟡 训练腿已闭环 | 训练腿 4 闭环 / 0 失败（L2_PARAM→上抛、L1_RESOURCE→重试、L4→probe 恢复，业务均继续）；推理腿进行中。发现：真实超时为进程级致命，L3 重放不成立 → 需进程外编排（监控方向） |
...
| 2 | **错误注入 → 恢复闭环**（验收标准 3，设备侧原语 + 真实证据） | 🟡 训练腿完成；推理腿待复跑（OOM 注入改渐进式） |
