...
| 本次新增覆盖 | **S-7 图捕获**、**S-16 流数量配额（2000 流）** |
| ⚠️ 判据口径更正（2026-09-22 晚） | S-7 的探针已把「捕获区内切流」从判据**降为宽容度观察项**（该写法在上游契约之外，正解是 `graph(g, stream=s)`）；判定只看契约内 4 项。MLU590 在观察项上**容忍**，P800 **不容忍** —— 属**运行时宽容度差异，非能力差异**。详见 §3.1 与 `../P800/docs/KUNLUN_P800_STREAM_BASELINE_16_20260920.md` §3.2.1 |
...
| **S-7** | 图捕获流语义 | ✅ | 图捕获探针 V2 | **契约内 4/4 通过**（G1/G2/G3/G5，rel_err 全 **0.00e+00**）+ 1 项**契约外用法**观察项（MLU **容忍**）。⚠️ 判据口径系 2026-09-22 更正，见 §3.1 |
...
### 3.1 S-7 图捕获：契约内 **4/4 通过**（+ 1 项宽容度观察项）

⚠️ **2026-09-22 晚更正**：本节的判据分组已调整 ——
「捕获区内切流」**在上游契约之外**（捕获区内的工作应留在捕获流上；要用副流必须经
`graph(g, stream=s)` 或显式 fork/join 把它纳入捕获），故从**判据**降级为**宽容度观察项**，
不计入 PASS/FAIL。判定的口径是**契约内 4 项**：
...
| G4* | 捕获区内切到未纳入捕获的流（**契约外用法**，观察项） | ✅ **容忍**（rel_err 0.00e+00）—— 与 P800 相反 |

> **P800 的对照**：同一条观察项在 P800（XPytorch）上**不容忍**
>（`AcceleratorError: CUDA error: unrecognized error code`，独立进程稳定复现）。
> ⇒ 该差异是**运行时宽容度**差异，**不是**"谁支持图捕获"的能力差异。

**本节数值来源说明（如实）**：上表 G1–G5 的数值来自本报告首次跑出的归档 JSON
（`probes/graph_capture_stream_result_mlu590_20260922.json`，五项均通过、rel_err 全 0），
按新口径**复算**即"契约内 4/4 通过 + G4 容忍"。
⚠️ **口径调整后的新鲜证据待补**：2026-09-22 晚间两台寒武纪主机 SSH 均超时
（同期 P800 / 910C 正常），故未能在新探针上重跑；恢复后补跑并归档（见 §7 待办）。
...
| 6 | **S-7 新口径的新鲜证据** | 探针判据分组调整后（G4 降为观察项）需在 MLU590 上重跑一次并归档；**2026-09-22 晚间寒武纪两台主机 SSH 超时**，待网络恢复补跑 |
| 7 | 合入节奏 | 按约定本批内容**随 10 月统一提交**（PR 前置条件：不对 910C/P800 既有结论造成回归——MLU590 侧已复验离线自检 38/0、smoke 46/0、conformance 13+6） |
