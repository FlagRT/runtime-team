...
【诚实标注】底层框架若未暴露事件句柄/统一错误对象，用例退化为
...
def case_s1_stream_order(ctx):
    ...
    x = torch.randn(32, 32, device=ctx["device"])
...
def case_s2_explicit_dependency(ctx):
    ...
    a = torch.randn(16, 16, device=ctx["device"])
    b = torch.randn(16, 16, device=ctx["device"])
...
def case_e1_event_record_wait(ctx):
    """E1 事件记录/等待：事件代表流进度（后端接口缺口时退化为同步近似）。"""
    try:
        ev = ctx["event"]()
        ev.record()
        ev.wait()
    ...
    x = torch.randn(8, 8, device=ctx["device"])
    ctx["sync"]()
    return True, "接口缺口：后端未暴露统一事件句柄，以同步近似验证"
...
def case_e2_wait_before_record(ctx):
    ...
    昇腾实测：wait 未 record 的事件可能永久阻塞（无超时）——统一语义要求
    等待确定 + 超时逃生（wait_host），该边界由统一层收敛。
    """
    try:
        ev = ctx["event"]()
        ev.record()
        ev.wait()
        ctx["sync"]()
        return True, "record→wait 正常路径通过；边界：wait 未 record 事件可能永久阻塞（无超时），需统一层收敛（如实标注）"
    except Exception as e:
        return False, f"统一事件 API 异常: {e}"
    return True, "接口缺口：后端未暴露统一事件句柄"
...
def case_t1_pinned_async_copy(ctx):
    ...
    dst = src.to(ctx["device"], non_blocking=True)
    ctx["sync"]()
    return _eq(dst.cpu(), src), "pinned->device non_blocking 拷贝数据一致"
...
def case_r_recovery(ctx):
    ...
    from recovery import (
        evaluate_device, recover_device,
    ...
    try:
        ...
        # R2 评估 + R3 隔离
        ev = evaluate_device(0, device=ctx["device"], sync_fn=ctx["sync"])
        set_device_state(0, DeviceState.ISOLATED, "conformance: L4 simulate")
        ok1 = query_device_state(0) == DeviceState.ISOLATED
        # R4 重建
        rec = recover_device(0, device=ctx["device"], sync_fn=ctx["sync"])
