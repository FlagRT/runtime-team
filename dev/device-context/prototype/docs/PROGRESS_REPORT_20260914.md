...
| **P800 · 阶段 2 训练腿** | 🔄 **进行中·集合通信已跑通** | ✅ flagcx all_reduce/all_gather/P2P **三类全 OK**（卡 6,7）；训练已启动（见 §2.4） |
| **P800 · 阶段 3 推理腿** | ⏸ **未启动** | — |
| **P800 · 阶段 4 错误闭环** | ⏸ **未启动** | — |
| **P800 · 阶段 5 产出与 release** | ⏸ **未启动** | 产出：《新芯片接入手册》+ 规范修订建议 |

**一句话**：910C 主干已闭环进入规模扩展阶段；P800 已完成「环境 → 基线 → 接入」三步，
**阶段 2 的集合通信已跑通（flagcx 三类全 OK）**，训练腿正在跑；
过程中曾误判为「flagcx 在 P800 上不可用」，已被卡 6,7 的复测推翻（见 §2.4.4）。
...
| A2 | **训练腿**：loss 单调下降、无 NaN、吞吐记录、通信三类对照 | 🔄 **已启动**（卡 6,7，50 步） |
...
| **P2** | 收训练腿 50 步结果；随后启动**推理腿**（与训练腿并行，权重对等） | 验收标准 4、5 的证据 |
