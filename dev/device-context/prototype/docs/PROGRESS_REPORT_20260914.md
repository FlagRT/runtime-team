...
| **P800 · 阶段 2 训练腿** | 🔄 **进行中·已定位到具体卡点** | ✅ flagcx 三类通信 **在探针与训练脚本中均通过**；❌ 卡在**逐参数梯度 all_reduce**（见 §2.4.7） |
| **P800 · 阶段 3 推理腿** | ⏸ **未启动** | — |
| **P800 · 阶段 4 错误闭环** | ⏸ **未启动** | — |
...
**下一步判定（已启动）**：`probes/kunlun/dc_probe_ar_rep.py`
- **C)** 对**同一个** 1024×1024 张量重复 all_reduce **200 次**，每 10 次打印 → 看是否挂在某个第 N 次
- **D)** 再对 310 个**真实梯度**张量按序 all_reduce，每 5 个打印 → 看是否复现 B#2

判据：若 C 在某个 N 挂住 ⇒「**重复次数**」是变量（累积性问题）；
若 C 跑完 200 次而 D 复现 B#2 ⇒「**张量序列**」是变量（特定张量问题，可进一步缩到该张量）。
...
| ~~C3~~ | ~~flagcx/BKCL 集合通信在 P800 上失败~~ | ❌ **撤销** —— 被 T5b 推翻，非缺陷 |
