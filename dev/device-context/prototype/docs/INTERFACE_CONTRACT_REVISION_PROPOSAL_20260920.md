...
> 版本：v1.0 ｜ 日期：2026-09-20 ｜ 提出方：device-context 子方向（Kistich）
> **对象**：`prototype/docs/INTERFACE_CONTRACT_DC_20260908.md`（现行 v0.1.0 定稿）
> **由来**：该约定是**昇腾单一实例**时期定稿的。第 2 家（昆仑芯 P800）接入是对它的**首次非昇腾检验**，
> 下面 6 条都是在这次检验中**实际暴露或首次被意识到**的问题。
> **写法**：每条给出 ①现状 ②问题（实测依据，可独立阅读）③建议条文（可直接抄进约定）④兼容性影响。
> 其中第 1–4、6 条有实测依据；第 5 条为**前瞻性条款**（本次未触发），已如实标注。
...
- **第 1、2、4 条**建议在 **9 月内**落进契约文本（第 4 条代码已修，第 2 条含一处具体缺陷需一并修）；
- **第 3、6 条**建议随 **v0.2（10 月）**发布；
- **第 5 条**留作 v0.2 的候选，视第 3 家芯片的实际情况决定是否必要。
