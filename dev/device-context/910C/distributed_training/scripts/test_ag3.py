import sys, ctypes
# 让 libflagos.so 的符号全局可见（GetCurrentStream/SetCurrentStream）
...
print(f"[t3] rank{rank}: out={[x.item() for x in out]}", flush=True)
...
print(f"[t3] rank{rank}: out2={[x.item() for x in out2]}", flush=True)
