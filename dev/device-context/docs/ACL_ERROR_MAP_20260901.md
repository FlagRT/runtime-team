...
> **口径注记（2026-09-02 复核）**：上表 `103/159 = 64.8%` 为 2026-09-01 快照，指"命中 CANN 头文件提取的 159 个码全集"的比例；`errors.py` 实际映射条目为 **108 条**（多出的 5 个属其他域，如 161xxx 基础段，不在 159 全集内）。两者口径不同均正确。
> 2026-09-02 重跑审计（`results_p3/acl_error_map_audit_latest.json`）：覆盖 103/159 = 64.8%，分级不一致 18 个（11.3%），其中高置信差异 3 个为**有意覆盖规则**（AICPU/VECTOR_CORE/FFTS_PLUS_EXCEPTION 上提 L4 以对齐 AICORE_EXCEPTION），**勿回退**。
> 权威全景见 `docs/ASCEND_910C_DC_STREAM_MAPPING_20260902.md` §3 D10。
，继续堆覆盖率需要用无依据的猜测填充，反而可能引入错误。
