...
set -u
# OUT / TAG 用于**多镜像等价性对照**：不同镜像跑同一脚本时把结果分开存放（默认行为不变）
OUT=${OUT:-/workspace/out_infer}
TAG=${TAG:-}
mkdir -p "$OUT"
exec > "$OUT/F_infer_leg${TAG}.log" 2>&1
...
env CUDA_VISIBLE_DEVICES=$DEV \
    ...
    DC_OUT_DIR="$OUT" \
    FLAGCX_ADAPTOR=klx \
    timeout $BUDGET python3 $PROTO --backend kunlun --out "proto_infer_leg_result${TAG}.json"
RC=$?

echo
if grep -q "INFER_LEG_PASS" "$OUT/proto_infer_leg_result${TAG}.json" 2>/dev/null; then
  echo "  >>> ✅ 推理腿 PASS"
elif grep -q "INFER_LEG_FAIL" "$OUT/proto_infer_leg_result${TAG}.json" 2>/dev/null; then
