...
2. **分配器画像(显存池第一层)复现通过**:缓存复用、同尺寸池命中、大块切分、OOM 重试 5 项全绿,行为与 [allocator-画像报告-20260817](profile_allocator画像_910c.md) 一致。碎片冗余绝对值因块尺寸缩放(见 §3)不可直接对比,语义判定不受影响。
...
## 3. 验证结果(探针:dev/memory/probes/legacy-2.4-910c/allocator-profile_910c.py)
...
- 脚本:dev/memory/probes/common/comm-smoke_flagcx.py + probes/910c/comm-hccl-direct_910c.py(纯 ctypes HCCL 对照)
