...
> 前置参考：[纯MoE-昆仑芯-20260822.md](纯MoE-昆仑芯-20260822.md)（旧线 4 参/5 参 TypeError 失败）、[新线镜像-MoE复测-静态预检-20260822.md](新线镜像-MoE复测-静态预检-20260822.md)（gems 5.x 全系未补 renormalize）
...
1. **生成质量**：eager 输出首 token 正确后退化重复 —— expert GEMM（xtorch_ops 内核）或 attention/采样链路
   存在数值问题；需数值层对照定位（本次未深挖）。
