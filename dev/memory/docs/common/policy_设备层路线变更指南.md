...
> ## ⛔ 现行结论：生产交付不走 `torch_fl` 设备层
> 各芯片统一使用厂商官方设备插件；上层继续使用 FlagGems、FlagCX、vllm-plugin-FL 等 FlagOS 组件。`torch_fl` 设备层路线**已冻结**：不承担交付责任、不作前置依赖、memory 子方向不再投入。相关历史资产见 [../goals/legacy-2.4-910c/](../goals/legacy-2.4-910c/README.md)。
...
已投入的验证资产（昇腾 Qwen3-4B TP=1/2/4、16 卡 DDP、FlagCX 补丁、若干上游 PR）作为历史资产保留于 [../goals/legacy-2.4-910c/](../goals/legacy-2.4-910c/README.md)。该路线不承担交付责任、不作为任何团队的前置依赖，memory 子方向不再沿此方向开发。上游 PR 是否继续跟进由相关人员按需处理，与本子方向交付脱钩。
...
> 运行时组的交付分两条线：
>
> **生产交付走路线 A**——各芯片使用厂商官方设备插件，其上使用 FlagGems、FlagCX、vllm-plugin-FL 等 FlagOS 组件；不使用 `torch_fl` 作为设备层。该组合有官方镜像、官方安装流程与上层组件 CI 支撑，昇腾与昆仑芯均已实测跑通 dense 推理，训练链路也只在这条线上存在。
>
> `torch_fl` 的既有验证资产保留归档，上游贡献按相关人员需要处理，不作为 memory 当前开发线，也不占用交付关键路径。它曾在路线 B 中承载显存池、进程组等运行时能力，但不是 FlagOS 各层的共同基础——上层组件对其零引用，各芯片设备层现状均为厂商插件。若未来重新评估，须按晋升门槛单独决策。
