...
> 来源：资料库在线文档《统一设备抽象封装_路线A与B总结_20260922》
> （https://www.workbuddy.cn/space/d/QI0l2zqO1dha1Mzbbd5cZ2 ；2026-09-22 同步，**已逐字核对内容一致**）
> 依据：`统一设备抽象封装与多芯片接入方案_20260918.md`（路线与方案）+ 两实例实测证据（截至 2026-09-22）
> 芯片口径：华为昇腾 910C（已完成）· 昆仑芯 P800（已完成）· 寒武纪（即将）；平头哥 PPU 备选
...
**路线 B = torch_fl**，即 FlagOS 侧的统一加速器抽象框架（组内定位为 **B 线 / 预研**）。

### 1.2 关于路线 B 的调研结论（为什么用不了）

| # | 问题 | 调研依据 |
|---|---|---|
| 1 | **覆盖不全，"通吃"前提不成立** | torch_fl 平台矩阵与 CI 配置实有 9 家：`cuda / metax / ascend / ppu / dcu / gcu / musa / bpu / tsingmicro`。其中**命中我们本期名单的只有 `ppu`（平头哥）一家**，且状态为 **Experimental**；**没有 cambricon（寒武纪）、没有 biren（壁仞）**。所谓"没适配的芯片就走 torch_fl"这条路本身走不通 |
...
| 4 | **它是预研支线，不承担交付** | 组内 A/B 线定义：A 线（厂商官方插件）＝生产交付主线；B 线（torch_fl）＝预研，不承担交付 |
| 5 | **生态默认适配对象是厂商栈** | 上层框架（vLLM / DeepSpeed / FlagGems）与厂商栈的适配由厂商持续推进，我们跟随即可 |

### 1.3 两条路线逐项对照

| 判据 | 路线 A（厂商官方栈） | 路线 B（torch_fl） |
|---|---|---|
| 覆盖本期三芯片 | ✅ 三家均有官方插件 | ❌ 仅平头哥（Experimental） |
| 与厂商栈共存 | ✅ 不冲突（本身就是厂商栈） | ❌ 争 PrivateUse1，同进程互斥 |
| 性能 | ✅ 原生（实测 5428 tok/s） | ❌ 差 2.3 倍（实测 2324 tok/s） |
| 交付属性 | ✅ 生产主线 | ⚠️ 预研支线，不承担交付 |
| 生态适配跟随度 | ✅ vLLM / DeepSpeed / FlagGems 的默认对象 | ⚠️ 低 |
| 长期维护 | ✅ 厂商持续维护 | ⚠️ 组内预研投入 |
...
```text
做：  统一设备抽象与执行上下文 · 插件机制与接入规范 · 接入手册与验收判据
不做：厂商设备插件实现与算子分发（厂商）· 通信库实现（通信方向）· 模型转换与算子库（编译/算子方向）

跨芯片边界：一个进程内只激活一家芯片；跨芯片切换在进程之间进行
   ⇒ "三家芯片通过插件后端接入，切换只改 runtime.use() 一行；
      同一进程激活一家，进程间可切换。"
```
...
| conformance | ✅ 13/13 + 推理 6/6（`ascend` / `flagos` 两后端均绿） |
...
| 训练腿（2 卡分布式微调） | ✅ 6/6 · loss **15.4497 → 11.1515**（50 步）· 2212.9 tok/s · 通信三类对照全对 |
| 推理腿（单卡前向 / 服务化） | ✅ 前向 **14/14**（79.4 句/s · p50 37.7 ms · 区分度 0.638）；服务化 10/10（**108 句/s** · p50 27.4 ms） |
| 错误注入 → 恢复闭环 | ✅ 推理腿 5 闭环 / 0 失败；训练腿 4 闭环 / 0 失败 |
...
| 项 | 状态 | 结果 |
|---|---|---|
| 阶段 0–1 环境 + 接入 | ✅ | `kunlun` backend 落地；**单日完成接入**（目标 ≤5 人天）；conformance **13/13 + 6/6**；smoke **42 通过 / 0 失败** |
...
| 厂商侧缺陷 | ⚠️ 已定性上报 | KL3 事件同步概率性挂死（**16/18 ≈ 89%**），根因**定位到函数级**（3 处自旋点均在厂商 `libxpucuda.so`）；算子层与编译层已硬证据排除，且**镜像因素亦已排除** |
...
| 已接入后端 | 3 个（`ascend` / `flagos` / `kunlun`），各 **306 / 293 / 478 行** |
...
| 跨后端缺陷 | **5 例**"只有非首实例才暴露"的框架缺陷，全部修在**框架层**（第 5 例即 9-22 对称复跑暴露） |
...
```text
① 寒武纪接入（第 3 家，≤5 人天）
   环境访问（当前：两台测试机 TCP 22 可达、认证凭据待配）
        → 环境普查（preflight_env.sh 一键出报告）
        → backends/cambricon/ 落地（name="cambricon" / device_type="mlu" / vendor="cambricon"）
        → conformance 13 + 6 → 多流 16 项逐项比对
        → 训练腿 2 卡 DDP（FlagCX + CNCL）
        → 产出并入接入手册 SOP + 接口修订建议
   顺带验证：接口约定修订建议第 1 条（device_type 与 vendor 分离）
              —— `mlu` 是第三种命名空间，是首次真实检验多命名空间的场景
...
2. **接入成本**：第 2 家实测**单日完成接入**（目标 ≤5 人天，已提前达标）。
3. **验收判据**：统一的 conformance 判据集 + 多流 16 项基线 + 两条腿证据链；
